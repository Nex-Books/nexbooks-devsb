...
import {
  MdAdd, MdAttachFile, MdDelete, MdRefresh, MdUploadFile, MdVisibility,
} from 'react-icons/md';
...
export default function InvoicesPage() {
  ...
  const confirmDelete = (inv: Invoice, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleDelete = async () => {
    ...
    try {
      const headers = await getAuthHeader();
      const res = await fetch(`${API}/finance/invoices/${deletingInvoice.id}?user_id=${user.id}`, {
        method: 'DELETE',
        headers,
      });
      if (!res.ok) {
      ...
      toast({ title: 'Delete failed', description: (err as Error).message, status: 'error', duration: 3000 });
    } finally {
      setIsDeleting(false);
              ...
              <Tr key={inv.id} _hover={{ bg: 'gray.50' }}>
                ...
                <Td>
                  <Flex gap="4px">
                    ...
                    <Tooltip label="Delete invoice" placement="left">
                      <IconButton
      ...
      <AlertDialog isOpen={isDeleteOpen} leastDestructiveRef={deleteRef} onClose={onDeleteClose}>
        <AlertDialogOverlay backdropFilter="blur(4px)">
          <AlertDialogContent borderRadius="16px">
            <AlertDialogHeader fontSize="md" fontWeight="700">Delete Invoice</AlertDialogHeader>
            <AlertDialogBody>
              Are you sure you want to permanently delete invoice <strong>{deletingInvoice?.invoice_number}</strong>?
              This will also delete all line items. This action cannot be undone.
            </AlertDialogBody>
            <AlertDialogFooter gap="8px">
              <Button ref={deleteRef} onClick={onDeleteClose} size="sm" variant="ghost">Cancel</Button>
              <Button colorScheme="red" onClick={handleDelete} isLoading={isDeleting} size="sm" borderRadius="8px">
                Delete
              </Button>
            </AlertDialogFooter>
          </AlertDialogContent>
